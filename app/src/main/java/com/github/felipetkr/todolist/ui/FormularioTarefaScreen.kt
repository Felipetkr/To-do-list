package com.github.felipetkr.todolist.ui
...
import com.github.felipetkr.todolist.util.combinarDataHora
import com.github.felipetkr.todolist.util.extrairDataDoDatePicker
import com.github.felipetkr.todolist.util.paraMillisUtcDoDatePicker
...
fun FormularioTarefaContent(
    ...
    var titulo by remember(tituloInicial) {
        mutableStateOf(tituloInicial)
    }
    var descricao by remember(descricaoInicial) {
        mutableStateOf(descricaoInicial)
    }
    var temDataHora by remember(dataHoraInicial) {
        mutableStateOf(dataHoraInicial != null)
    }
    ...
    var ano by remember(dataHoraInicial) {
        mutableStateOf(calendarioInicial?.get(Calendar.YEAR))
    }
    var mes by remember(dataHoraInicial) {
        mutableStateOf(calendarioInicial?.get(Calendar.MONTH))
    }
    var dia by remember(dataHoraInicial) {
        mutableStateOf(calendarioInicial?.get(Calendar.DAY_OF_MONTH))
    }
    var hora by remember(dataHoraInicial) {
        mutableStateOf(calendarioInicial?.get(Calendar.HOUR_OF_DAY))
    }
    var minuto by remember(dataHoraInicial) {
        mutableStateOf(calendarioInicial?.get(Calendar.MINUTE))
    }

    var mostrarSeletorData by remember {
        mutableStateOf(false)
    }
    var mostrarSeletorHora by remember {
        mutableStateOf(false)
    }

    if (mostrarSeletorData) {
        val estadoDatePicker = rememberDatePickerState(
            initialSelectedDateMillis = if (ano != null) {
                paraMillisUtcDoDatePicker(ano!!, mes!!, dia!!)
            } else {
                null
            }
        )
        ...
        Column(
            ...
            Button(
                ...
                enabled = titulo.isNotBlank() &&
                        (!temDataHora || (ano != null && hora != null))
            ) {
                Text("Salvar")
            }
        }
